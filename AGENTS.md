...
Without `vars.PAZAAK_API_BASES`, Pages is **offline-static** (local AI playable; no `github.io/api/*` fetches). Live multiplayer API: `https://pazaak-matchmaking.bocloud.workers.dev` (repo var `PAZAAK_API_BASES`). See `docs/solutions/runtime-errors/github-pages-same-origin-api-bases.md`, `docs/solutions/runtime-errors/worker-match-me-proxy-match-actor.md`, and `docs/ops/holowan-oauth-and-api-bases.md`.
